...
bool Phonebook::remove(const std::string& first, const std::string& last)
{
    ...
    // std::unordered_map<std::string, long>::size_type count = m_map.erase(key);
    // oder
...
std::string wandleEintragNachNamen(std::pair<std::string, long> entry)
{
